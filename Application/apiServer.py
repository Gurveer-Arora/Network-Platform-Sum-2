"""Vehicle rental fleet API (Flask-RESTful).

Endpoints (all under /api/v1):
    GET     /vehicles                    list/filter vehicles              (public)
    GET     /vehicles/<id>               vehicle details                   (public; admin sees more)
    POST    /fleet                       add a vehicle to the fleet        (admin)
    DELETE  /fleet/<id>                  remove a vehicle from the fleet   (admin)
    POST    /rentals                     rent a vehicle, body {"vehicleId": n}   (public)
    POST    /returns                     return a vehicle, body {"vehicleId": n} (admin)

Authentication: not implemented yet. All auth lives in get_role() and
require_admin() (see the "Authentication / authorisation" section) so it can be
replaced with the admin-account logic later without touching the endpoints.
"""
import csv
import os
import tempfile
...
api.add_resource(VehicleCollection, f"{API_PREFIX}/vehicles")
api.add_resource(VehicleItem, f"{API_PREFIX}/vehicles/<int:vehicle_id>")
api.add_resource(FleetCollection, f"{API_PREFIX}/fleet")
api.add_resource(FleetItem, f"{API_PREFIX}/fleet/<int:vehicle_id>")
api.add_resource(RentalCollection, f"{API_PREFIX}/rentals")
api.add_resource(ReturnCollection, f"{API_PREFIX}/returns")

if __name__ == "__main__":
    app.run(debug=True)  # run in debug mode - not for production use